...
<section class="section">
    <div class="section-header">
        <h1>{{ isset($product) ? '✏️ Editar Produto' : '➕ Novo Produto' }}</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('admin.products.index') }}">Produtos</a></div>
            <div class="breadcrumb-item">{{ isset($product) ? 'Editar' : 'Novo' }}</div>
        </div>
    </div>
                        ...
                        <form action="{{ isset($product) ? route('admin.products.update', $product->id) : route('admin.products.store') }}" 
                            ...
                            <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control @error('stock') is-invalid @enderror"
                                                       id="stock" name="stock" value="{{ old('stock', $product->stock ?? 1) }}" required>
                                                @error('stock')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control @error('condition') is-invalid @enderror"
                                                    ...
                                                    <option value="novo" {{ old('condition', $product->condition ?? '') === 'novo' ? 'selected' : '' }}>Novo</option>
                                                    <option value="seminovo" {{ old('condition', $product->condition ?? '') === 'seminovo' ? 'selected' : '' }}>Seminovo</option>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group">
                                        <img id="imagePreview" 
                                             src="{{ isset($product) && $product->image ? asset($product->image) : asset('uploads/no-image.png') }}"
                                             alt="Preview" 
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save"></i> {{ isset($product) ? 'Atualizar Produto' : 'Salvar Produto' }}
                                </button>
